---
title: Schedule
format:
  html:
    page-layout: full
    code-fold: true
    echo: false
    execute: true
---

Talks will be 30 minutes long with a break of 10 minutes for discussion.

<!-- 
## 17 June

9:30 Opening

9:40

10:20

11:00 Coffee Break

11:30 

12:10

12:50 Lunch break

15:00

15:40

16:20

## 18 June

9:00 

9:40

10:20

11:00 Coffee Break

11:30 

12:10

12:50 Lunch break


15:00

15:40

16:20

## 19 June

9:00 

9:40

10:20

11:00 Coffee Break

11:30 

12:10 -->


The registration will be open on 17 June at 9:00, and at 9:30 we will have the opening of the conference. 
The room is 124 Anile, located in the Department of Mathematics and Computer Science of the University of Catania.

Blackboard and overhead projector will be available for the talks (but not both simultaneously).

Coffee breaks will be at 11:00 and lunch will be between 13:00 and 15:00.

:::{#talks-list}


```{ojs}
MathJax = {
  const MathJax = await require('https://cdn.jsdelivr.net/npm/mathjax@4/tex-svg.js')
    .catch(() => window.MathJax)
  return MathJax
}


wm= window['MathJax'] = {
  tex: {
    inlineMath: {'[+]': [['$', '$'], ['\\(', '\\)']]}
  },
  output: {
        font: 'mathjax-fira',
        displayOverflow: 'scroll'
  }
};



//file = FileAttachment("talks.xlsx").xlsx() 
//data = file.sheet("talks", {range:"A1:G21",headers: true})

// data = [
//   {Name: "Alice", Affiliation: "University A", Title: "Talk on $x^2$"},
//   {Name: "Bob", Affiliation: "University B", Title: "Talk on $y^2$"}
// ]


chairs = ["Pedro A. García-Sánchez (morning), Francesco Strazzanti (afternoon)", "Julio José Moyano Fernández (morning), Lorenzo Guerrieri (afternoon)", "Hailong Dao (morning)"];


dataf = FileAttachment("talks.tsv").tsv()

data=dataf.sort((a, b) => a.Day.localeCompare(b.Day) || a.Hour.localeCompare(b.Hour))

function isEmptyOrSpaces(str){
    return str === null || str.match(/^ *$/) !== null;
}

muestra = function(){
var code = "<div id='talks'>"
var day=""
var i=0
data.forEach(d => {
if (d.Day !== day) {
  day = d.Day;
  code = code + "<h3>" + day.toLocaleString("en-UK", {
  day: "numeric",
  month: "long",
  year: "numeric"
}) + "</h3>"
code = code + "<p><strong>Chairs:</strong> " + chairs[i] + "</p>"
i++
}
code = code +"<p> "+ d.Hour
if (!isEmptyOrSpaces(d.Name)) {
  code = code + " <strong>"+ d.Name +"</strong>"
}
if (!isEmptyOrSpaces(d.Affiliation)) {
  code = code + ", " + d.Affiliation
}else{
  console.log("No affiliation for " + d.Name)
}
if (!isEmptyOrSpaces(d.Title)) {
  code = code + ", <em>" + d.Title + " </em></p>"
}
if (!isEmptyOrSpaces(d.Slides)) {
  code = code + "<p><a href='slides/" + d.Slides + ".pdf' target='_blank'>Slides</a></p>"
}
if (!isEmptyOrSpaces(d.Abstract)) {
  code = code + "<details><summary>Abstract</summary><p>" + d.Abstract + "</p></details>"
}
code = code + "</div>"
});

return code
}

//html`${data.map(d => html`<p>${d.Name} ${d.Affiliation} ${d.Title}</p>`)}`

//html`<p>${data[0].Title.val}</p>`

// blah ="$x^2$"

// ble = data[0].Title

// blu = ble + " $x^2$"

// example = html`${blah}`

// ex = html`${blu}`

html`${muestra()}`

```

:::

<!-- 
<script>
window.MathJax = {
  tex: {
    inlineMath: {'[+]': [['$', '$'], ['\\(', '\\)']]}
  }
};
MathJax.typesetPromise(["talks-list"])
</script> -->
